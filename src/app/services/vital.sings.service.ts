    ...
    if (rolId == Roles.FAMILIAR) {
      this.doctorService.getDoctorsByPatientId(patient.patient_id).subscribe((response) => {
        ...
        const dataEmail: EmailVitalSignsData = {
          ...
          state_hearth_rate: EnumEStatusPatient[this.calculateStatusForHearthRate(vitalSigns)],
          color_hearth_rate: this.calculateColorByStatusVitalSign(this.calculateStatusForHearthRate(vitalSigns)),
          blood_pressure: vitalSigns.blood_pressure.toString(),
          state_blood_pressure: EnumEStatusPatient[this.calculateStatusForBloodPressure(vitalSigns)],
          color_blood_pressure: this.calculateColorByStatusVitalSign(this.calculateStatusForBloodPressure(vitalSigns)),
          o2_saturation: vitalSigns.O2_saturation.toString(),
          state_o2_saturation: EnumEStatusPatient[this.calculateStatusForO2Saturation(vitalSigns)],
          color_o2_saturation: this.calculateColorByStatusVitalSign(this.calculateStatusForO2Saturation(vitalSigns)),
        }
